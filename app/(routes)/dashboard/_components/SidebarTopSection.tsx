...
import { Teams } from "./Sidebar";

interface SidebarTopSectionProps {
  user: any;
  activeTeam: Teams;
  setActiveTeam: React.Dispatch<React.SetStateAction<Teams>>;
}
  ...
  return (
    <TooltipProvider>
      <div>
            ...
            <div>
              {menu.map((item) => (
                <h2
                  ...
                  className="items-center flex gap-2 mt-2 text-sm hover:bg-slate-100 rounded-md p-1 cursor-pointer"
                >
        ...
        <Button
          variant="outline"
          className="w-full justify-start gap-2 bg-black text-white font-bold mt-8 p-4"
        >
